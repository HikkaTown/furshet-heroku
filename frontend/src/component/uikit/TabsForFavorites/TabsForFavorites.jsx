...
function TabsForFavorites({onClick, buffetsLength, gastroLength, masterclassLength, barLength, category}) {
  return (
    <div className={s.container}>
      <NavigationButton onClick={() => {
        onClick('Фуршет')
      }} text={'Фуршет'} length={buffetsLength}
                        className={cs(s.btn, category === 'Фуршет' && s.btn_active)}/>
      <NavigationButton onClick={() => {
        onClick('Гастро-станции')
      }} text={'Гастро-станции'} length={gastroLength}
                        className={cs(s.btn, category === 'Гастро-станции' && s.btn_active)}/>
      <NavigationButton onClick={() => {
        onClick('Мастер-классы')
      }} text={'Мастер-классы'} length={masterclassLength}
