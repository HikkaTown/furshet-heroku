...
export default function Index({data}) {
  ...
  const checkCategoryFavorites = async (category) => {
    ...
    if (category === 'Фуршет') {
      ...
      data = await fetch('http://localhost:3000/api/getStations').then((res) => {
        return res.json()
      });
      data.map((item) => {
      ...
      data = await fetch('http://localhost:3000/api/getMasterClass').then((res) => {
        return res.json()
      });
      data.map((item) => {
      ...
      data = await fetch('http://localhost:3000/api/getBarCards').then((res) => {
        return res.json()
      });
      data.map((item) => {
  ...
  return (
    <>
      <Layout>
        ...
        <div className={s.container}>
          {!(favoritesMasterClass.length !== 0 || favoritesGastro.length !== 0 || favoritesBuffets.length !== 0 || favoritesBar.length !== 0) ? (
            <>
              <p className={s.title}>
            ...
            <div className={s.block}>
              ...
              />
              <CardsBlockFavorites data={visibleCards} categoryName={category}/>
            </div>
          )}
        </div>
      </Layout>
